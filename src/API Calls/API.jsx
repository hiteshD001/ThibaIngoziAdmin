...
export const useGetPermissionsByRoleId = (roleId) => {
    const navigate = useNavigate();

    const queryFn = async () => {
        return await apiClient.get(`${import.meta.env.VITE_BASEURL}/super-admin/permission/${roleId}`);
    };
...
export const useGetChartData = (company_id, time, startDate, endDate, notificationType, ehailing = false) => {
    const queryFn = async () => {
        ...
        if (notificationType) {
            ...
            // Default type for ehailing payload
            params.type = '677b75b538c4008959469d0c';
        }
    ...
    const res = useQuery({
        queryKey: ["chartData", notificationType, company_id, time, startDate, endDate, ehailing], // Re-fetch when dependencies change
        queryFn: queryFn,
        staleTime: 15 * 60 * 1000,
    });
...
export const useGetRoles = (page = 1, limit = 10000) => {
    const queryFn = async () => {
        return await apiClient.get(`/role-management/roles`, { params: { page, limit } });
    };
...
// get all permissions
export const useGetPermissions = (page = 1, limit = 20) => {
    const queryFn = async () => {
        return await apiClient.get(`/role-management/permissions`, { params: { page, limit } });
    };
...
export const useGetPermission = () => {
    ...
    const queryFn = async () => {
        return await apiClient.get(`${import.meta.env.VITE_BASEURL}/super-admin/permission`);
    };
...
// get side data for dashboard
export const useGetSideData = (page = 1, limit = 10) => {
    const queryFn = async () => {
        return await apiClient.get('/role-management/side-data', { params: { page, limit } });
    };

    const res = useQuery({
        queryKey: ["sideData", page, limit],
        queryFn: queryFn,
        staleTime: 15 * 60 * 1000,
        retry: false,
    });
...
export const useGetCompanyUsers = (companyId) => {
    const queryFn = async () => {
        return await apiClient.get(`/role-management/company-users/${companyId}`);
    };
...
// 8.2 Toggle Permission Status
export const useTogglePermission = (onSuccess, onError) => {
    return useMutation({
        mutationFn: async ({ permissionId, status }) => {
            const response = await apiClient.put(`/super-admin/permission/toggle/${permissionId}`, { status });
            return response.data;
        },
        onSuccess,
        onError,
