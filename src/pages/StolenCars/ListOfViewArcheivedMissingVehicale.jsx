...
const ListOfViewArcheivedMissingVehicale = () => {
    ...
    const achiveMissingPerson = usePatchArchivedMissingVehicale(
        () => {
            toast.success("Stolen Car archived successfully!")
    ...
    const handleExport = async ({ startDate, endDate, exportFormat: fileFormat }) => {
        try {
            const { data } = await apiClient.get(`${import.meta.env.VITE_BASEURL}/missingVehicle`, {params: {}});

            const allUsers = data?.data || [];
            if (!allUsers.length) {
                toast.warning("No Stolen Car data found.");
                return;
            }
            ...
            if (fileFormat === "xlsx") {
                ...
                XLSX.utils.book_append_sheet(workbook, worksheet, "Missing_Vehicles_List");
                XLSX.writeFile(workbook, "Missing_Vehicles_List.xlsx");
            }
                ...
                link.download = 'Missing_Vehicles_List.csv';
                ...
                autoTable(doc, {
                    startY: 30,
                    head: [["SOS Id", "Last Seen Location", "Date", "Request Reached", "Request Accepted", "Status", "Reported By"]],
                ...
                doc.save("Missing_Vehicle_List.pdf");
            }
    ...
    const handleView = (url) => {
        saveScrollPosition("missingCarListScroll");
        nav(url);
    };
    useEffect(() => {
        if (MissingPersons.data?.data?.totaldata) {
            restoreScrollPosition("missingCarListScroll");
        }
                                            ...
                                            <TableRow key={obj?._id}>
                                                ...
                                                <TableCell sx={{ color: '#4B5563' }}>
                                                    <Stack direction="row" alignItems="center" spacing={1}>
                                                        {obj.evidence_image.slice(0, 2).map((item, index) => {
                                                        ...
                                                        {obj.evidence_image.length > 2 && (
                                                            <Box
                                                                ...
                                                                onClick={() => handleView(`/home/total-stolen-cars/stolen-car/${obj._id}`)}
                                                            >
                                                                +{obj.evidence_image.length - 2}
                                                            </Box>
                                                        )}
                                                ...
                                                <TableCell >
                                                    <Box align="center" sx={{ display: 'flex', flexDirection: 'row' }}>
                                                        <Tooltip title="View" arrow placement="top">
                                                            <IconButton onClick={() => handleView(`/home/total-stolen-cars/stolen-car/${obj._id}`)}>
