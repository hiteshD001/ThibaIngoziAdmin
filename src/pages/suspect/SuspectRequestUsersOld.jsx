...
import { useGetSuspectSightingsReportedUsers } from "../../API Calls/API";
...
const SuspectRequestUsers = () => {
    ...
    const locationData = useGetSuspectSightingsReportedUsers(locationId, page, rowsPerPage,debouncedFilter);
    const location = locationData.data?.data;

    // Access user list and pagination data from response
    const userList = location?.users;
    const totalCount = location?.total || 0;
    const totalPages = location?.totalPages || 0;
    const currentPageFromServer = location?.page || 1;
            ...
            <Paper elevation={1} sx={{ backgroundColor: "rgb(253, 253, 253)", mb: 2, padding: 2, borderRadius: '10px' }}>
                ...
                <Grid container justifyContent="space-between" alignItems="center" mb={2}>
                    <Grid size={{ xs: 12, lg: 3 }} sx={{ display: 'flex', flexDirection: 'row', gap: 2, mb: { xs: 1, md: 0 } }}>
                        <Typography variant="h6" fontWeight={590}> Suspect Sightings Reported</Typography>
                    </Grid>
                                ...
                                displayUsers.map((user, index) => (
                                    <TableRow key={user._id || index}>
                                        <TableCell sx={{ color: '#4B5563' }} >
                                            <Stack direction="row" alignItems="center" gap={1} >
                                                <Avatar
                                                    src={getImageLink(user.selfieImage) || nouser}
                                    ...
                                    <TableCell sx={{ color: '#4B5563', borderBottom: 'none' }} colSpan={4} align="center">
                                        <Box sx={{ py: 4, textAlign: 'center' }}>
                                            ...
                                            <Typography variant="body2" color="text.secondary">
                                                No users were reached for this suspect sightings.
                                            </Typography>
                                        </Box>
                                    </TableCell>
                                </TableRow>
                        ...
                        <Box display="flex" alignItems="center" gap={{ xs: 1, sm: 2 }}>
                            ...
                            >
                                <NavigateBeforeIcon fontSize="small" sx={{
                                    color: !location?.hasPrev ? '#BDBDBD !important' : '#1976d2 !important'
                                }} />
                            </IconButton>
                            <IconButton
                            ...
                            >
                                <NavigateNextIcon fontSize="small" sx={{
                                    color: !location?.hasNext ? '#BDBDBD !important' : '#1976d2 !important'
                                }} />
                            </IconButton>
                        </Box>
                    </Grid>
