...
import { useUpdateMarkAsReviewed } from "../../API Calls/API";
...
const Confirmation = () => {
    ...
    const policedata = location.state?.police_unit_details;
    const nav = useNavigate()

    const onSuccess = () => {
        toast.success("Status Updated Successfully.");4
    ...
    const markAsReviewed = (id, report_status, police_unit_id = "") => {
        mutate({
            id: id,
            data: { report_status: report_status, police_unit_id },
        })
    }
            ...
            <Paper elevation={2} sx={{ p: 3, mt: 3, mb: 3, borderRadius: '10px' }}>
                ...
                <Box sx={{ px: 3, backgroundColor: "#F9FAFB", borderRadius: '10px', p: 3, my: 4 }}>
                    <React.Fragment>
                        ...
                        <Box sx={{ py: 1, }}>
                            <Typography fontSize="1rem" >
                                <img src={earth} alt="confirmation icon" /> Sending to All Police Units
                            </Typography>
                ...
                <Box sx={{ my: 4, display: 'flex', justifyContent: 'center' }}>
                    <Button variant="contained" sx={{ height: '48px', width: '240px', borderRadius: '8px', fontWeight: 500, backgroundColor: 'var(--Blue)' }} onClick={() => markAsReviewed(data?._id, 'With SAPS', policedata)}>
                        Confirm and Send
                    </Button>
                </Box>
            </Paper>
